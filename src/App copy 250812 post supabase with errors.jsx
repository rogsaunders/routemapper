  ...
  try {
    ...
    try {
      ...
      if (newWindow) {
        ...
        if (newWindow) {
          return { success: true, method: "new_window" }; // Ensure function exits after success
        } else {
    ...
    try {
      ...
      return new Promise((resolve) => {
        reader.onload = () => {
          ...
          resolve({ success: true, method: "data_url" }); // Exit function after success
        };
    ...
    async function tryShareSheet(env, blob, filename, title) {
...
export default function App() {
  ...
  const exportAsJSON = async (
    ...
    try {
      ...
      // Add the code that should be inside the try block here
    try {
      // Add the code that should be inside the try block here
    } catch (error) {
...
const handleEndstage = async () => {  // <-- ADD 'async' here
  try {
    setstageStarted(false);
    setUndoTimeLeft(5);
    ...
    if (user && user !== 'guest') {
      try {
        ...
        // Save tracking points if available
        ...
        setSyncStatus('error');
        // Don't throw - continue with local export even if cloud save fails
      }
    }
    // ========= END SUPABASE INTEGRATION =========

    // Export files locally (your existing export code)
    const exportResults = [];
    ...
    // Clear data for next stage
    setRefreshKey((prev) => prev + 1);
    setIsTracking(false);
    localStorage.removeItem("unsavedWaypoints");
    
    // Clear current route ID if starting new route
    if (waypoints.length === 0) {
  ...
  if (user) {
      const saveData = async () => {
        ...
        if (trackingPoints.length > 0) {
          await dataSync.saveTrackingPoints(trackingPoints, stageId);
        }
