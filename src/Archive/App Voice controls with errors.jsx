...
const exportFileIPadCompatible = async (
  ...
  try {
    ...
    console.error(`âŒ Export failed for ${filename}:`, error);
...
function Home() {
  ...
  useEffect(() => {
    ...
    const handleError = (err) => {
      console.error("âŒ GPS error", err);
      setGpsLoading(false);
    ...
    const interval = setInterval(() => {
      if (currentGPS?.lat && currentGPS?.lon) {
        navigator.geolocation.getCurrentPosition(
          ...
          (err) => console.error("âŒ GPS error", err),
  ...
  const handleNewRoute = () => {
    ...
    console.log(
      `ðŸ—ºï¸ Started Route ${currentRoute + 1} for Day ${currentDay}`
    );
  };
  ...
  const handleStartstage = () => {
    ...
    const geo = navigator.geolocation;
    if (!geo) {
      console.error("âŒ Geolocation not supported");
    ...
    geo.getCurrentPosition(
      ...
      (err) => {
        console.error("âŒ Failed to get GPS:", err);
  ...
  const processVoiceCommand = (transcript) => {
    ...
    if (
      ...
      console.log(
        "ðŸŸ¢ Mic On command detected - starting continuous listening"
      );
  ...
  const exportAsJSON = async (
    ...
    try {
      ...
      console.error("âŒ Enhanced JSON export failed:", error);
      throw error;
    }
  };
  ...
  const exportAsSimpleJSON = async (
    ...
    try {
      ...
      console.error("âŒ Simple JSON export failed:", error);
      throw error;
    }
  };
  ...
  const exportAsGPX = async (
    ...
    try {
      ...
      console.error("âŒ GPX export failed:", error);
      throw error;
    }
  };
  ...
  <gpx version="1.1" creator="RallyMapper-Voice" xmlns="http://www.topografix.com/GPX/1/1">
    ...
    <trk>
      ...
      console.error("âŒ Rally Navigator GPX export failed:", error);
      throw error;
    }
  };
  ...
  const exportAsKML = async (
    ...
    try {
      ...
      console.error("âŒ KML export failed:", error);
      throw error;
    }
  };
  ...
  const handleEndstage = async () => {
    try {
      ...
      try {
        ...
        console.error("âŒ Sequential export failed:", error);
      }
      ...
      if (successCount === exportResults.length) {
        ...
        setGpsError("âŒ All exports failed. Check console for details.");
      }
      ...
      console.error("âŒ handleEndstage error:", error);
      setGpsError("âŒ Stage end failed. Check console.");
