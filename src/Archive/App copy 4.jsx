  ...
  function Home({ user, isGuestMode }) {
    ...
    useEffect(() => {
      if (waypointListRef.current) {
        waypointListRef.current.scrollTop =
          waypointListRef.current.scrollHeight;
      }
    }, [routeWaypoints]);
    ...
    const handleAddWaypoint = () => {
      if (!currentGPS) {
        setGpsError(
          "No GPS signal available. Please wait for GPS to be ready."
        );
        return;
      }
    ...
    const processVoiceCommand = (transcript) => {
      ...
      if (
        ...
        console.log(
          "🟢 Mic On command detected - starting continuous listening"
        );
    ...
    const deleteSelectedWaypoints = () => {
      ...
      const indicesToDelete = Array.from(selectedWaypoints).sort(
        (a, b) => b - a
      );
  ...
  <gpx version="1.1" creator="RallyMapper-Voice" xmlns="http://www.topografix.com/GPX/1/1">
    ...
    const handleEndstage = async () => {
      try {
        ...
        const summary = {
          ...
          endTime:
            routeWaypoints[routeWaypoints.length - 1]?.timestamp || "N/A",
          totalDistance: routeWaypoints
            .reduce((sum, wp) => sum + parseFloat(wp.distance || 0), 0)
            .toFixed(2),
          pois: [
            ...new Set(routeWaypoints.map((wp) => wp.poi).filter(Boolean)),
          ],
          ...
          try {
            ...
            if (!routeId) {
              const route = await dataSync.saveRoute({
                routeName:
                  routeName || `Day ${currentDay} Route ${currentRoute}`,
        ...
        // ==== Derived route data and UI helpers (moved inside Home) ====
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {routeWaypoints.length !== 1 ? "s" : ""} and start fresh. Make
                  sure you've exported your current data first.
                </p>
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  Export files and stop tracking for this stage.
                </p>
                <div style={{ display: "flex", gap: "12px" }}>
                  <button
    ...
    return (
      <div className="min-h-screen">
        {/* Put your existing Map + controls + panels here */}
        {/* Example:
        <GoogleMap ...>
          ...
        </GoogleMap>
        */}
        ...
        {/* === Moved your summary card INSIDE Home's return === */}
        <div>
          <div className="flex justify-center items-center gap-4 my-4 flex-wrap">
            ...
            <button
              ...
              style={{
                ...
                color: "white",
                cursor:
                  !currentGPS || !stageStarted ? "not-allowed" : "pointer",
                border: "2px solid #1e3a8a",
              }}
            >
            ...
            <button
              onClick={
                stageStarted ? startVoiceInput : handleGlobalVoiceCommands
              }
              type="button"
